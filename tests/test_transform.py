...
def test_epistemic_gold_concept_joins_name(policy: EpistemicOntologyPolicy) -> None:
    ...
    concept_lf = pl.LazyFrame(
        {
            ...
            "source_id": ["1", "2"],
            "concept_name": ["Name 1", "Name 2"],
            "domain_id": ["Condition", "Drug"] # Simulating an extra OMOP column being retained
        }
    )
    ...
    # Assert
    assert len(df) == 2
    assert "source_id" not in df.columns  # Ensure redundant ID is dropped
    assert "synonyms" in df.columns       # Ensure array is present
    assert "domain_id" in df.columns      # Ensure rich Silver data is retained

    row1 = df.filter(pl.col("coreason_id") == "concept-1").row(0, named=True)
    assert row1["name"] == "Name 1"
    assert sorted(row1["synonyms"]) == ["Term A", "Term B"]

    row2 = df.filter(pl.col("coreason_id") == "concept-2").row(0, named=True)
    assert row2["synonyms"] == ["Term C"]
...
def test_epistemic_gold_concept_missing_description(policy: EpistemicOntologyPolicy) -> None:
    ...
    row1 = df.filter(pl.col("coreason_id") == "concept-1").row(0, named=True)
    assert row1["name"] == "Name 1"
    assert row1["synonyms"] == [] # Should be an empty list, not null
