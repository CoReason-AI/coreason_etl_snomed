...
def test_rxnorm_configuration_state_valid() -> None:
    ...
    with patch.dict(
        ...
        assert config.bronze_data_path == "data/bronze/rxnorm/raw"
        assert config.log_level == "INFO"


def test_rxnorm_configuration_state_missing_api_key() -> None:
    ...
    with patch.dict(os.environ, {"DB_URI": "postgresql://user:pass@localhost:5432/coreason_db"}, clear=True):
        with pytest.raises(ValidationError) as exc_info:
            EpistemicRxNormPolicy()

        errors = exc_info.value.errors()
        assert any(e["loc"] == ("umls_api_key",) for e in errors)


def test_rxnorm_configuration_state_missing_db_uri() -> None:
...
def test_rxnorm_configuration_state_invalid_log_level() -> None:
...
def test_rxnorm_configuration_state_property_based(api_key: str, db_uri: str) -> None:
